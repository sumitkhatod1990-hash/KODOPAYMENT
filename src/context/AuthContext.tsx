...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    apiClient('/api/v1/auth/me').then((res) => res.ok ? res.json() : null).then((data) => setUser(data?.user || null)).catch(() => setUser(null)).finally(() => setLoading(false));
  }, []);
  const signIn = async (email: string, password: string) => {
    try {
      const res = await apiClient('/api/v1/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password }) });
  ...
  const signUp = async (details: { name: string; company: string; email: string; password: string }) => {
    try {
      const res = await apiClient('/api/v1/auth/signup', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(details) });
      const data = await res.json();
  ...
  const signInWithGoogle = async (credential: string) => {
    try {
      const res = await apiClient('/api/v1/auth/google', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ credential }) });
      const data = await res.json();
  ...
  const signOut = async () => { await apiClient('/api/v1/auth/logout', { method: 'POST' }).catch(() => undefined); setUser(null); };
