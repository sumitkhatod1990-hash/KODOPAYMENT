...
export const DeveloperTab: React.FC = () => {
  ...
  return <div className="max-w-5xl space-y-7">
    ...
    <section className="rounded-3xl bg-white dark:bg-[#0c0f17] border border-black/10 dark:border-white/10 p-6 space-y-4"><div className="flex items-center gap-3"><Code2 className="w-5 h-5 text-blue-600" /><div><h3 className="font-bold">Create a checkout session</h3><p className="text-xs text-slate-500">Call this from your server to get a shareable checkout link for a product (supports multi-currency: INR, USD, EUR, GBP, AED, AUD, CAD, SGD, JPY).</p></div></div><div className="rounded-2xl bg-slate-950 text-slate-100 p-4 overflow-x-auto"><pre className="text-xs leading-6">{`POST /api/v1/payments/create-session\nAuthorization: Bearer qivro_test_... (or qivro_live_...)\nContent-Type: application/json\n\n{\n  "productId": "prod_...",\n  "currency": "USD" // optional, defaults to product currency or INR\n}`}</pre></div></section>
