...
export const PaymentsTab: React.FC = () => {
  ...
  const handleRefund = async (txId: string) => {
    if (confirm('Are you sure you want to issue a full refund for this transaction under QIVROPAY Platform?')) {
      setRefundingId(txId);
      const result = await processRefund(txId);
      setRefundingId(null);
      if (!result.success) {
        alert(`Refund failed: ${result.error || 'Please try again.'}`);
      }
    }
  };

  const handleCheckRefundStatus = async (txId: string) => {
    setCheckingId(txId);
    const result = await checkRefundStatus(txId);
    setCheckingId(null);
    if (!result.success) {
      alert(`Could not check refund status: ${result.error || 'Please try again.'}`);
    }
  };
    ...
    <div className="space-y-8 animate-fade-in">
          ...
          {(['all', 'succeeded', 'refunded'] as const).map(status => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-semibold capitalize transition-all ${
                statusFilter === status
                  ? 'bg-[#1d1d1f] dark:bg-white text-white dark:text-[#0c0f17] shadow-sm'
      ...
      {transactions.length === 0 && (
        <div className="p-10 sm:p-14 rounded-3xl bg-white border border-black/10 shadow-sm text-center space-y-4">
          ...
          <div>
            <h3 className="font-bold text-[#1d1d1f] text-base font-sans">No payments yet</h3>
            <p className="text-xs text-[#86868b] mt-1 max-w-sm mx-auto">
              Payments will appear here after your first customer completes checkout.
            </p>
          </div>
          <button
                  ...
                  <tr key={tx.id} className="hover:bg-[#f5f5f7] transition-colors">
                    ...
                    <td className="p-4">
                      {tx.status === 'refund_pending' ? (
                        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-700 dark:text-blue-300 font-bold text-[10px] border border-blue-200">
                        ...
                        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-amber-50 text-amber-700 font-bold text-[10px] border border-amber-200">
                          ...
                          {tx.status === 'partially_refunded' ? 'Partially Refunded' : 'Refunded'}
                        </span>
                      ) : (
                        <div className="flex flex-col gap-0.5">
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 font-bold text-[10px] border border-emerald-200 w-fit">
                            <CheckCircle2 className="w-3 h-3" />
                            Succeeded
                          </span>
                          {(tx.refundStatus === 'FAILED' || tx.refundStatus === 'CANCELLED') && (
                            <span className="text-[10px] text-red-600 font-bold">Last refund attempt failed</span>
                          )}
                        </div>
                      )}
                    </td>
                    <td className="p-4">
                      <div className="flex items-center gap-2">
                        ...
                        {tx.status === 'succeeded' && (
                          ...
                          >
                            {refundingId === tx.id ? 'Refunding...' : (tx.refundStatus === 'FAILED' || tx.refundStatus === 'CANCELLED') ? 'Retry Refund' : 'Refund'}
                          </button>
                        )}

                        {tx.status === 'refund_pending' && (
                          <button
                            onClick={() => handleCheckRefundStatus(tx.id)}
                            disabled={checkingId === tx.id}
                            className="text-xs text-[#86868b] hover:text-[#0071e3] hover:underline"
                          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-md animate-fade-in">
          <div className="w-full max-w-md rounded-3xl bg-white border border-black/10 shadow-2xl p-6 sm:p-8 space-y-5">
            <div className="flex justify-between items-center pb-3 border-b border-black/5">
              <div className="flex items-center gap-2">
                <span className="font-bold text-[#1d1d1f] text-base font-sans">Payment Receipt</span>
                <span className={`text-[10px] font-mono px-2 py-0.5 rounded-full font-bold border ${
                  selectedTx.status === 'refund_pending' ? 'bg-blue-50 text-blue-700 dark:text-blue-300 border-blue-200' :
                  selectedTx.status === 'refunded' || selectedTx.status === 'partially_refunded' ? 'bg-amber-50 text-amber-700 border-amber-200' :
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-[#1d1d1f] font-mono uppercase">{selectedTx.country}</span>
              </div>
              <div className="flex justify-between">
              ...
              {selectedTx.refundId && (
                <div className="pt-3 border-t border-black/5 space-y-1.5">
                  <div className="flex justify-between">
                    <span className="text-[#86868b]">Refund Status:</span>
                    <span className="text-[#1d1d1f] font-mono font-bold">{selectedTx.refundStatus || (selectedTx.status === 'refund_pending' ? 'PENDING' : '—')}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono text-[#1d1d1f]">{selectedTx.refundId}</span>
                  </div>
                  {(selectedTx.refundConfirmedAt || selectedTx.refundRequestedAt) && (
                    <div className="flex justify-between">
                      <span className="text-[#86868b]">Refund {selectedTx.refundConfirmedAt ? 'Confirmed' : 'Requested'}:</span>
                      <span className="text-[#1d1d1f] font-mono">{new Date(selectedTx.refundConfirmedAt || selectedTx.refundRequestedAt || '').toLocaleString()}</span>
                    </div>
                  )}
                </div>
              )}
